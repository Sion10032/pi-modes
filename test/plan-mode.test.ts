...
test("tool selection allows safe built-ins and non-built-ins only", () => {
	...
	assert.equal(
		canSelectToolInPlanMode(extensionTool("custom") as PlanTool),
		true,
	);
	assert.deepEqual(
		withRequiredPlanModeTools(["read", "plan_mode_question", "read"]),
		["read", "plan_mode_question"],
	);
	assert.deepEqual(
		withoutPlanModeQuestionTool(["read", "plan_mode_question"]),
		["read"],
	);
});
...
test("proposed-plan helpers extract and remove plan blocks", () => {
	...
	assert.equal(
		stripProposedPlanBlocks("A<proposed_plan>secret</proposed_plan>B"),
		"AB",
	);
	assert.deepEqual(
		stripProposedPlanBlocksFromMessage({
