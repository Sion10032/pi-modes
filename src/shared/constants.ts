// 共享常量：工具白名单、黑名单、bash 安全策略等

/** 在受限模式下允许的内置工具白名单 */
export const SAFE_BUILTIN_TOOLS = new Set([
	"read",
	"bash",
	"grep",
	"find",
...
/** 在受限模式下允许的只读 bash 命令模式 */
export const SAFE_BASH_PATTERNS = [
	/^\s*(cd|cat|head|tail|less|more|grep|find|ls|pwd|echo|printf|wc|sort|uniq|diff|file|stat|du|df|tree|which|whereis|type|env|printenv|uname|whoami|id|date|uptime|ps|jq|awk|rg|fd|bat|eza)\b/i,
	/^\s*sed\s+-n\b/i,
